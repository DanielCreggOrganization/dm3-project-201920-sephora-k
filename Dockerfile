...
RUN apt-get update && apt-get -y install apache2 mysql-server php libapache2-mod-php php-mysql php-curl php-gd php-mbstring php-xml php-xmlrpc phpmyadmin
...
RUN grep '^Include /etc/phpmyadmin/apache.conf' /etc/apache2/apache2.conf && echo conf already there || sudo sed -i '$ a Include /etc/phpmyadmin/apache.conf' /etc/apache2/apache2.conf